"""Tests for ServiceClient against a real HTTP server (no mocks).

`http.server` is used in a thread with an ephemeral port, the same as
the sibling libraries (`httptest.NewServer` in Go, Fastify in Node with
`port: 0`): ServiceClient is given a real `base_url` and makes real
socket requests, exercising raise_for_status() and the connection pool
just as the 20 generated services will use them.
"""
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: object) -> None:  # noqa: A002
        pass  # silences the access log of the test server
...
@pytest.fixture
def run_server():
    """Start a real http.server on 127.0.0.1 with an ephemeral port per test."""
